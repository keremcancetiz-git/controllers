...
|-----------|--------------------------------------|-------------------------------------------------------|
| in        | `<base_topic>/collect-bkg`                   | `{"experiment_name": "<name>", "experiment_name_suffix": "<suffix>"}` (`experiment_name_suffix` optional — appended to the exported filename) |
| out       | `<base_topic>/collect-bkg/response`          | `{"status": "success"\|"error"\|"busy", ...}`         |
| in        | `<base_topic>/collect-sample`                | `{"experiment_name": "<name>", "experiment_name_suffix": "<suffix>"}` (`experiment_name_suffix` optional — appended to the exported filename) |
| out       | `<base_topic>/collect-sample/response`       | `{"status": "success"\|"error"\|"busy", ...}`         |
| in        | `<base_topic>/experiment-folders`            | `{}` (empty)                                          |
| out       | `<base_topic>/experiment-folders/response`   | `{"status": "success", "folders": [[name, ctime]]}`   |
| in        | `<base_topic>/get-absorbance`                | `{"experiment_name": "...", "wavenumber": 1234.5}`    |
| out       | `<base_topic>/get-absorbance/response`       | `{"status": "success", "results": [{timestamp, absorbance}]}`|

`<base_topic>` defined in `config.json`.
